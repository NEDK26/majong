...
2. 双击 EXE，默认浏览器会打开“牌理镜”本地研究面板；
3. 选择显示器，在预览图上拖框圈住自己的暗牌区域，然后开始分析。

使用完请点击面板底部的“退出程序”，以同时停止浏览器后台的本地 EXE。

Windows Defender 可能会对新发布、未签名的个人 EXE 显示信誉提示；可以先在
Release 页面核对来源。程序只启动 `127.0.0.1` 本地服务，不向公网传输截图。
...
实时面板只捕获你框选的屏幕区域，服务仅监听 `127.0.0.1`。它不读取游戏
进程、不抓包、不注入，也不控制鼠标键盘或发送网络请求。

首次使用：

1. 安装 Python 3.10 或更新版本，并勾选 **Add Python to PATH**；
2. 把雀魂 34 种牌总览图拖到 `calibrate_mahjong_soul.bat` 上完成本机校准；
3. 双击 `start_live_viewer.bat`，浏览器会打开“牌理镜”研究面板；
4. 选择显示器，在预览图上拖框圈住自己的暗牌区域，然后点击“开始实时观察”。

单显示器使用时，可以点击“3 秒后抓取”，随后切回游戏；三秒后面板会取得
游戏画面，切回浏览器即可框选。运行过程中也可以切回游戏，浏览器后台会继续
轮询本机服务。建议使用窗口化游戏或把面板放在第二显示器。
...
如果 Windows 防火墙弹窗出现，无需允许公网访问；程序只绑定本机回环地址。
若预览是黑屏，请把游戏切换为窗口化或无边框窗口后重试。
