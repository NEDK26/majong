...
from tile_utils import tile_index_to_name, validate_34_array
...
def _effective_tiles(hand_13: list[int], shanten: int) -> tuple[tuple[EffectiveTile, ...], int]:
    """枚举 34 种摸牌，找出能严格降低向听数的牌。"""
    validate_34_array(hand_13, allowed_totals={13})
    effective: list[EffectiveTile] = []

    for draw_index in range(34):
        # 自己手中已有四枚时，不可能再摸到同种牌。
        if hand_13[draw_index] >= 4:
            continue

        drawn_hand = hand_13.copy()
        drawn_hand[draw_index] += 1
        if calculate_shanten(drawn_hand) < shanten:
            # 按需求只扣除当前手牌，不扣除任何场上可见牌。
            remaining = 4 - hand_13[draw_index]
            effective.append(
...
def core_analyze(hand_tiles: list[int]) -> AnalysisResult:
    ...
    - 14 张：逐种模拟舍牌，计算舍牌后的向听数、有效进张和理论枚数。
    - 13 张：此时规则上尚无合法舍牌动作，因此返回当前有效摸牌；摸到第
      14 张后再次调用本函数，即可得到候选舍牌。
